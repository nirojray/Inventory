...
public partial class ReportPurchaseRegister : System.Web.UI.Page
{
        ...
        try
        {
            if (TxtFromDate.Text != string.Empty && TxtToDate.Text != string.Empty)
            {
                ...
                if (fromdate <= Todate)
                {
                    ...
                    DataTable dt1 = ClsReportSalesRegister.ReportPurchaseRegister(StartDate, EndDate);
                    if (dt1.Rows.Count > 0)
                    {
                        ds.Tables.Add(dt1);


                        ds.Tables[0].TableName = "Purchase Register";


                        ExcelHelper.ToExcel(ds, "Purchase_Register.xls", Page.Response);
                    }
                    else
                    {

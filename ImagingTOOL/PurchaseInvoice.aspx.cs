...
public partial class PurchaseInvoice : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            Txt_FromDate.Text = GetDefaultStartDate(Convert.ToDateTime(currentDate).AddDays(-14));
        ...
        try
        {
            if (e.CommandName == "Edit1")
            {
                ...
                if (drModel.Length > 0)
                {
                    ...
                    TxtSODate.Text = dtNew.Rows[0]["PO_Date"].ToString();
                    TxtSoNo.Text = dtNew.Rows[0]["PO_Number"].ToString();
    ...
    //            bool result = ClsPurchaseInvoice.InsertPurchaseInvoice(Convert.ToInt32(asd), Convert.ToDateTime(Txt_InvoiceDate.Text.Trim()), Txt_InvoiceNo.Text.Trim(), Convert.ToInt32(Session["Id"]));
    //            if (result)
    //            {

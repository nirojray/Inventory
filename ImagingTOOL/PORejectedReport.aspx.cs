...
public partial class PORejectedReport : System.Web.UI.Page
{
        ...
        try
        {
            if (TxtFromDate.Text != string.Empty && TxtToDate.Text != string.Empty)
            {
                ...
                if (fromdate <= Todate)
                {
                    ...
                    DataTable dt1 = ClsReportSalesRegister.ReportPurchaseOrderRejected(StartDate, EndDate);
                    if (dt1.Rows.Count > 0)
                    {
                        ds.Tables.Add(dt1);


                        ds.Tables[0].TableName = "Purchase Order Reject";


                        ExcelHelper.ToExcel(ds, "Purchase_Order_Reject.xls", Page.Response);
                    }
                    else
                    {

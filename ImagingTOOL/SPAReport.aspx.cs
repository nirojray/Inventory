...
public partial class SPAReport : System.Web.UI.Page
{
        ...
        try
        {
            if (TxtFromDate.Text != string.Empty && TxtToDate.Text != string.Empty)
            {
                ...
                if (fromdate <= Todate)
                {
                    ...
                    DataTable dt1 = ClsReportSalesRegister.ReportSPA(StartDate, EndDate);
                    if (dt1.Rows.Count > 0)
                    {
                        ds.Tables.Add(dt1);


                        ds.Tables[0].TableName = "SPA Report";


                        ExcelHelper.ToExcel(ds, "SPA_Report.xls", Page.Response);
                    }
                    else
                    {

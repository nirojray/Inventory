...
public partial class MaterialDispatch : System.Web.UI.Page
{
    ...
    private void BindMainGrid()
    {
        ...
        try
        {
            DT = ClsSalesManagerChecking.Get_SalesOrderApproved(Convert.ToDateTime(Txt_FromDate.Text.Trim()), Convert.ToDateTime(Txt_ToDate.Text.Trim()));
        ...
        try
        {
            if (e.CommandName == "Edit1")
            {
                ...
                if (drModel.Length > 0)
                {
                    ...
                    LoadddlDispatch();
                    if (dtNew.Rows[0]["DispatcDetails"].ToString() != null && dtNew.Rows[0]["DispatcDetails"].ToString() != "")
                    {
                        ddlDispatchDetails.SelectedValue = dtNew.Rows[0]["DispatcDetails"].ToString();
                        if (ddlDispatchDetails.SelectedItem.ToString() != "HD")
                        {
                            TRTransporter.Visible = true;
                            LoadddlTransporter();
                            ddlTransporter.SelectedValue = dtNew.Rows[0]["NameOfTheTransporter"].ToString();
                        }
                        else
                        {
                    ...
                    {
                        TRTransporter.Visible = false; Clear();
                    }
                    ...
                    if (dtNew.Rows[0]["DateOfDelivery"].ToString() != null && dtNew.Rows[0]["DateOfDelivery"].ToString() != "")
                    {
                        Txt_DateOfDelivery.Text = GetDefaultStartDate(Convert.ToDateTime(dtNew.Rows[0]["DateOfDelivery"].ToString()));
                    }
                    else
                    {
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        bool result = ClsSalesManagerChecking.InsertDispatchDetails(Convert.ToInt32(ddlDispatchDetails.SelectedValue),Convert.ToDateTime(Txt_Dispatch_Date.Text.Trim()),
            txtAWBNo.Text.Trim(), DateOfDelivery, Int_transporter, Convert.ToInt32(Session["Id"]),Convert.ToInt32(txtSOID.Text),Convert.ToInt32(Session["DCID"].ToString()));
        if (result)
        {
            Clear();
            BindMainGrid();

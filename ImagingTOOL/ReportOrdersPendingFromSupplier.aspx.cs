...
public partial class ReportOrdersPendingFromSupplier : System.Web.UI.Page
{
    ...
    protected void BtnExport_Click(object sender, EventArgs e)
    {
        if (TxtFromDate.Text != string.Empty && TxtToDate.Text != string.Empty)
        {
            ...
            if (fromdate <= Todate)
            {
                ...
                DataTable dt1 = ClsReportOrderPending.Get_OrderPending(StartDate, EndDate);
                if (dt1.Rows.Count > 0)
                {
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt1);
                    ds.Tables[0].TableName = "OrderPendingDetails";
                    ExcelHelper.ToExcel(ds, "Order_Pending_From_Supplier.xls", Page.Response);
                }
                else
                {
